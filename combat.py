...
def ChangerDePokemon(Joueur: joueur):
    ...
    Joueur.affiche_pokemons_captures(Joueur)
    remplacant = input(
        "Choisissez un nouveau Pokemon"
    )  # à voir plus tard quand on commencera l'interface graphique
    index = 0
...
def combat(joueur: joueur, pokemon_sauvage: pokemonSauvage):
    ...
    if pokemon_sauvage.hp <= 0:
        ...
        # backtomap
        # game over gros nul
    elif fuite(pokemon_sauvage, joueur.pokemons_captures[0]):
        # backtomap
        ...
        if msgCallBack_An():
            if pokemon_sauvage.vitesse < joueur.pokemons_captures[0].vitesse:
                pokemon_sauvage.hp -= attaqueneutre(
                    joueur.pokemons_captures[0], pokemon_sauvage
                )
                if pokemon_sauvage.hp <= 0:
                    joueur.capture(pokemon_sauvage)
                else:
                    joueur.pokemons_captures[0].hp -= attaqueneutre(
                        pokemon_sauvage, joueur.pokemons_captures[0]
                    )
            else:
                joueur.pokemons_captures[0].hp -= attaqueneutre(
                    pokemon_sauvage, joueur.pokemons_captures[0]
        ...
        if msgCallBack_At():
            if pokemon_sauvage.vitesse < joueur.pokemons_captures[0].vitesse:
                pokemon_sauvage.hp -= attaquetype(
                    joueur.pokemons_captures[0], pokemon_sauvage
                )
                if pokemon_sauvage.hp <= 0:
                    joueur.capture(pokemon_sauvage)
                else:
                    joueur.pokemons_captures[0].hp -= attaquetype(
                        pokemon_sauvage, joueur.pokemons_captures[0]
                    )
            else:
                joueur.pokemons_captures[0].hp -= attaquetype(
